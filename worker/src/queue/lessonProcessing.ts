...
import { updateLessonStatus } from '../utils/database';
...
export async function processLessonTranscoding(job: Job<LessonProcessingJobPayload>) {
  const { lessonId, s3Key, filePath } = job.data;

  console.log(`[${job.id}] Starting lesson processing for lesson ${lessonId}`);
  ...
  try {
    ...
    console.log(`[${job.id}] Lesson status updated to PROCESSING`);
  } catch (error) {
    console.error(`[${job.id}] Failed to update status to PROCESSING:`, error);
    // Continue processing even if status update fails
  }

  const tempDir = path.join(process.env.TEMP_DIR || '/tmp/transcode', lessonId);
  ...
  try {
    ...
    const transcodeResult = await runTranscode(inputFile, outputDir, lessonId);
    console.log(`[${job.id}] Transcoding completed`);

    // Step 3: Save HLS files
    let hlsMaster: string;
    if (STORAGE_MODE === 'local') {
      // Save to local storage
      const { getHlsDirPath, getHlsUrlPath } = await import('../services/localStorage');
      const hlsDir = getHlsDirPath(lessonId, undefined); // Lessons don't use category-based paths
      hlsMaster = getHlsUrlPath(lessonId, undefined);
      
      console.log(`[${job.id}] HLS output directory: ${hlsDir}`);
      console.log(`[${job.id}] HLS URL path: ${hlsMaster}`);
      ...
      const s3HlsPrefix = `hls/lessons/${lessonId}`;
      ...
      hlsMaster = `${s3HlsPrefix}/master.m3u8`;
    }

    // Step 4: Update database
    console.log(`[${job.id}] Updating database with hlsMaster: ${hlsMaster}`);
    const updatedLesson = await updateLessonStatus(lessonId, 'READY', hlsMaster);
    console.log(`[${job.id}] Database updated. Lesson status: ${updatedLesson.status}, hlsMaster: ${updatedLesson.hlsMaster}`);
    ...
    console.log(`[${job.id}] Lesson processing completed successfully for lesson ${lessonId}`);
    return {
      success: true,
      lessonId,
      hlsMaster,
    ...
    console.error(`[${job.id}] Lesson processing failed for lesson ${lessonId}:`, error);
    ...
    try {
      await updateLessonStatus(lessonId, 'UPLOADED', undefined);
    } catch (dbError) {
